resource "vsphere_virtual_machine" "virtual_machine" {
  ...
  guest_id = lookup(each.value, "template", null) != null ? data.vsphere_virtual_machine.template[lookup(each.value.template, "template_name", null)].guest_id : "ubuntu64Guest"
  ...
  dynamic "cdrom" {
    ...
    content {
      datastore_id = data.vsphere_datastore.datastore[lookup(each.value.iso, "datastore", null)].id
      path         = lookup(each.value.iso, "path", null)
    }
  }
